...
class WaypointNavigator(Node):
    ...
    def goal_response_callback(self, future):
        ...
        if not goal_handle.accepted:
            ...
            # Try next waypoint if current one is rejected
    ...
    def get_result_callback(self, future):
        try:
            ...
            self.get_logger().info(f'Navigation result: {status_map.get(status, "Unknown")}')
            ...
            if status == ActionGoalStatus.STATUS_SUCCEEDED:
                ...
                # Retry the same waypoint or move to next
                self.get_logger().warning(f'Navigation to waypoint {self.current_waypoint} failed')
                self.current_waypoint += 1
                self.navigate_to_next_waypoint()
